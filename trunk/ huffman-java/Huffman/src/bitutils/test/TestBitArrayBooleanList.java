...
/**
 * <code>TestBitArrayBooleanList</code> defines
 * a class test for the <code>BitArrayBooleanList</code> class
 * 
 * @author Franck Séhédic <sehedic@ecole.ensicaen.fr>
 */
public class TestBitArrayBooleanList {
	
	private byte[] testArray= { (byte)0xF0, 0x55 , (byte)0xF0, 0x55 };
	
	private BitArray newBitArray(){
		return new BitArrayBooleanList();
	}
...
*/
	...
	private BitArray newBitArray(byte[] ba,int nbBits){
		return new BitArrayBooleanList(ba,nbBits);
	}
	
	private BitArray newBitArray(byte[] ba,int nbBits,int bitOff){
		return new BitArrayBooleanList(ba,nbBits,bitOff);
	}
	
	private BitArray newBitArray(byte[] ba,int nbBits,int bitOff, int byteOff){
		return new BitArrayBooleanList(ba,nbBits,bitOff,byteOff);
	}
